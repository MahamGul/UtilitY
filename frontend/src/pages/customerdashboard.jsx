...
// Mock data for active requests
const activeRequests = [
  { id: 1, serviceType: "Plumbing", description: "Fix leaking kitchen sink", budget: "Rs. 3,000 - Rs. 5,000", status: "Pending", datePosted: "2 hours ago" },
  { id: 2, serviceType: "Electrical", description: "Install ceiling fan in bedroom", budget: "Rs. 4,000 - Rs. 7,000", status: "Accepted", datePosted: "1 day ago", provider: "Usman Ahmed" },
  { id: 3, serviceType: "Mechanical", description: "Car engine diagnostic and repair", budget: "Rs. 15,000 - Rs. 25,000", status: "In Progress", datePosted: "3 days ago", provider: "Hassan Ali" },
  { id: 4, serviceType: "Plumbing", description: "Bathroom shower head replacement", budget: "Rs. 2,000 - Rs. 3,500", status: "Completed", datePosted: "5 days ago", provider: "Imran Khan", completedDate: "2 days ago", rating: null },
  { id: 5, serviceType: "Electrical", description: "Fix circuit breaker issues", budget: "Rs. 5,000 - Rs. 8,000", status: "Completed", datePosted: "10 days ago", provider: "Ali Raza", completedDate: "7 days ago", rating: 5 }
];
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="p-8 space-y-8">
        {/* Quick Stats */}
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <StatCard icon={<Clock className="w-6 h-6 text-yellow-600" />} title="Pending" count={1} bg="bg-yellow-100" />
          <StatCard icon={<Clock className="w-6 h-6 text-purple-600" />} title="In Progress" count={1} bg="bg-purple-100" />
          <StatCard icon={<CheckCircle className="w-6 h-6 text-green-600" />} title="Completed" count={completedServices.length} bg="bg-green-100" />
          <StatCard icon={<Star className="w-6 h-6 text-orange-600" />} title="To Rate" count={unratedServices.length} bg="bg-orange-100" />
        </div>

        {/* Pending Ratings Alert */}
        {unratedServices.length > 0 && (
          <div className="bg-gradient-to-br from-orange-400 via-orange-300 to-yellow-300 rounded-2xl p-8 text-white shadow-lg flex justify-between items-center">
            <div>
              <h2 className="text-2xl font-bold text-gray-900">Rate Your Recent Services</h2>
              <p className="text-lg text-gray-800">
                You have {unratedServices.length} completed service{unratedServices.length > 1 ? 's' : ''} waiting for your feedback
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-8">
            ...
            <div className="mb-6 flex gap-2 justify-center">
              {[1,2,3,4,5].map((star) => (
                ...
                >
                  <Star
                    className={`w-12 h-12 ${star <= (hoveredRating || rating) ? 'fill-yellow-400 text-yellow-400' : 'text-gray-300'}`}
                  />
                </button>
              ))}
            </div>
...
function StatCard({ icon, title, count, bg }) {
  return (
    <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm">
      <div className="flex items-center gap-4">
